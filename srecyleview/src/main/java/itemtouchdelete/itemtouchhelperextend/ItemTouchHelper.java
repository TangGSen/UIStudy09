...
package itemtouchdelete.itemtouchhelperextend;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        //当滑动其他Item ,需要关闭上一个
        if(mPreSelected !=null && mPreSelected !=vh){
            closeOpenedPreItem();
        }
        return true;
    }
    ...
    public abstract static class Callback {
         ...
         * @param mPreSelected
         */
        public abstract void onItemClose(ViewHolder mPreSelected);
    }
    ...
    private View findConsumeView(ViewGroup parent, float rawX, float rawY) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            View child = parent.getChildAt(i);
            if (child instanceof ViewGroup && child.getVisibility() == View.VISIBLE) {
                View view = findConsumeView((ViewGroup) child, rawX, rawY);
                if(view!=null){
                    return view;
                }
            } else {
                //找到不是ViewGroup 但是需要判断该点是不是在该view上
                if (isInBounds(child, rawX, rawY)) {
                    return child;
                }
            }
        }
        //如果上面都找不到，可能就是本身
        if (isInBounds(parent, rawX, rawY)) {
            return parent;
        }
        return null;
    }

    private boolean isInBounds(View child, float rawX, float rawY) {
        int[] location = new int[2];
        child.getLocationOnScreen(location);
        Rect rect = new Rect(location[0], location[1], child.getWidth() + location[0], child.getHeight() + location[1]);
        //符合条件
        if (rect.contains((int) rawX, (int) rawY) && child.getVisibility() == View.VISIBLE &&
    ...
    private void closeOpenedPreItem() {
        ...
        objectAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                ...
                mCallback.onItemClose(mPreSelected);
            }
